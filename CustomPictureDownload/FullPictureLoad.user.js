...
(async () => {
    ...
    const customData = [{
        ...
        name: "秀人集",
        host: ["www.2xiuren.cc", "www.xr06.xyz", "m.xr06.xyz"],
        ...
        name: "秀人美女網",
        host: ["www.xrmn01.com", "m.xrmn01.com"],
        ...
        name: "中看图片大全/图片发达网/昆山美图网/桃子啦/漫爱美图/雾四图片网/屈求图库/闲人图片大全/天气图片大全/洒清图片网/宫戏图片网/有臣美图/MM美图网/克斯图片网/你给图库/质量美图网/茫茫图片库/黑丝MM图库/MM美图网/奇米美图777/几时图片网/夜寄图库/出卖图片库/所尚图片大全/80天堂图片网/玩偶图片网",
        ...
        reg: /^https?:\/\/www\.(zkjmpx|tufada|ksxx365|tzala|mash120|wslak|777url|xr70|t7mm|sqhyyz|gxwpjc|ycwlx|ksxx360|ngptp|zlsmm|mmdmlt|hsnmm|mmxsl|i9ke|jsjfgkgs|yjpfxs|cmylzx|sskge|iduobi|woxiutu)\.com\/\w+\/\d+\.html|^https?:\/\/www\.tufada\.com\/tu\d+\.html/,
...
document.body.appendChild(text);
        ...
        preloadNext: (nextDoc, obj) => {
            let arr = fun.getImgsSrcArr(obj.imgs, nextDoc);
        ...
        preloadNext: (nextDoc, obj) => {
            fun.iframeDoc(nextLink, ".main_img img", 30000).then(nextIframeDoc => {
                let arr = fun.getImgsSrcArr(obj.imgs, nextIframeDoc);
                fun.picPreload(arr, obj.customTitle(nextIframeDoc), "next");
            });
        },
        category: "comic"
        ...
        prev: 1,
        customTitle: doc => fun.title(" - ", 2, doc),
        preloadNext: true,
        category: "comic"
    }, {
        name: "動漫狂",
        ...
        init: "document.onkeydown=null;",
        imgs: doc => {
            let imgDir = fun.ge("img[onload]", doc).src.match(/.+\//)[0];
            let max = [...fun.gae("option[value]", doc)].length;
            return fun.arr(max).map((_, i) => imgDir + String((i + 1)).padStart(3, "0"));
        },
        button: [4],
        insertImg: ["//td[a[img[@oncontextmenu]]]", 2],
...
window.parent.postMessage({
    ...
    title: document.title
        ...
        imgs: doc => {
            let imgs = fun.getImgsSrcArr("#images img:not([src*=loading]),#scroll-image img", doc);
        ...
        imgs: doc => {
            if (/www\.manhuabaiku\.com/.test(location.host)) {
                ...
                return fun.getImgsSrcArr("img[data-original]:not([data-original*='/template/pc/default/']),.lazy-read:not([data-original*='/template/pc/default/']),img[data-src]", doc);
            }
        },
        button: [4],
        ...
        imgs: doc => fun.getImgsSrcArr(".comic-page img,img[data-src],img[data-original]", doc),
        autoDownload: [0],
        next: async () => {
        ...
        imgs: async doc => {
            ...
            */
            return fun.getImgsSrcArr(".touch-manipulation img", doc);
        },
        button: [4],
...
document.body.appendChild(text);
    ...
    const fun = {
        ...
        getImgsSrcArr: (img, doc = document) => {
            let imgs;
