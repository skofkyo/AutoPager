...
(async () => {
    ...
    const customData = [{
        ...
        imgs: async () => {
            ...
            return [...fun.gae(".images .imageContainer .image img")].map(e => {
                if (/imx\.to/.test(e.src)) {
                    return e.src.replace("https://imx.to/u/t/", "https://i.imx.to/i/")
                } else if (/imgadult\.com/.test(e.src)) {
                    return e.src.replace("small-medium/", "big/")
                }
            })
        },
        insertImg: [
