...
(async () => {
    ...
    const customData = [{
        ...
        imgs: ".separator>a",
        customTitle: () => {
            return fun.geT("h1.post-title").trim();
        },
        category: "nsfw1"
    }, {
        name: "www.4kup.net",
        ...
        css: ".CustomPictureBox{height:auto!important}",
        category: "comic"
    }, {
        ...
        css: "#content~a,.content~a,.apjg,.pager a:nth-child(n+2):nth-child(-n+3){display:none!important}.pager a{width:44%!important}",
        category: "comic"
    }, {
        name: "漫畫類 自動展開目錄",
        enable: 1,
    ...
    const imgZipDownload = async () => {
        ...
        if (fastDownload) {
            ...
            if (!siteData.autoDownload || siteData.autoDownload && siteData.autoDownload[0] != 1 && options.autoDownload != 1) {
                ...
                titleText = await prompt("請輸入自訂壓縮檔資料夾名稱", (customTitle || document.title.replace(/\[\d+p(\d+v)?\]|【\d+P】/i, "").replace(/[\/\?<>\\:\*\|":]/g, " ").trim()));
